    ...
    <div>
      <div class="max-w-2xl mx-auto">
        <div class="bg-white shadow-md rounded-lg max-w-sm dark:bg-gray-800 dark:border-gray-700">
          <a :href="product.url">
            <img class="rounded-t-lg p-8 h-[300px] object-cover" :src="product.image" :alt="product.title">
          </a>
          <div class="px-5 pb-5">
            <a :href="product.url">
              <h3 class="text-gray-900 font-semibold text-xl tracking-tight dark:text-white">{{ truncateTitle(product.title) }}</h3>
            </a>
...
<script>
  ...
  methods: {
    truncateTitle(title) {
      // truncate title if it exceeds certain length
